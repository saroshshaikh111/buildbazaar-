...
const PLATFORM_EMAIL = process.env.PLATFORM_EMAIL || 'onboarding@resend.dev';
...
export async function sendEscrowReleased(order) {
    ...
    await resend.emails.send({
        from: PLATFORM_EMAIL,
        to: ADMIN_EMAIL,
        subject: `✅ ESCROW RELEASED: ₹${Number(order.vendorPayout).toLocaleString('en-IN')} — #${order.id.slice(0, 8).toUpperCase()}`,
        html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
